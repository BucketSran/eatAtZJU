...
async function checkProfileContracts() {
  ...
  assert(tutorialOverlay.includes('feedbackPoints') && tutorialOverlay.includes('tutorial-feedback-list') && tutorialOverlay.includes("setPhase('success')"), 'tutorial overlay must stop after user action and show a concise feature feedback summary')
  assert(tutorialOverlay.includes('function locateCurrentTarget') && tutorialOverlay.includes('scrollIntoView') && tutorialOverlay.includes('tutorial-focus-pulse'), 'tutorial action button must locate and highlight the real target instead of running an abstract demo')
  assert(!tutorialOverlay.includes('demoElement.click()') && !tutorialOverlay.includes('window.dispatchEvent(new CustomEvent') && !tutorialOverlay.includes('TUTORIAL_REQUIREMENT_DEMO_EVENT'), 'tutorial must not simulate user actions through scripted demo events')
  assert(!globalCss.includes('tutorial-demo-toast') && !globalCss.includes('tutorial-demo-playing') && globalCss.includes('.tutorial-focus-pulse'), 'tutorial styles must remove auto-demo playback chrome and keep only target focus feedback')
  assert(!fs.readFileSync(path.join(root, 'src/constants/tutorialDemo.ts'), 'utf8').includes('TUTORIAL_REQUIREMENT_DEMO_DURATION_MS'), 'tutorial constants must not retain old scripted demo duration knobs')
  assert(homeRoute.includes('TUTORIAL_REQUIREMENT_PRESET') && !homeRoute.includes('tutorialDemoCue') && !homeRoute.includes('TUTORIAL_REQUIREMENT_DEMO_DURATION_MS'), 'home tutorial preset should provide defaults without scripted highlight animation state')
  ...
  assert(globalCss.includes('.tutorial-focus-pulse') && !/\\.tutorial-(?:scrim|state--success \\.tutorial-scrim)[\\s\\S]*?backdrop-filter:\\s*blur/.test(globalCss), 'tutorial locator must use a lightweight target pulse without background blur')
