...
async function checkProfileContracts() {
  ...
  assert(tutorialOverlay.includes('tutorial-demo-playing') && globalCss.includes('.tutorial-demo-playing .tutorial-popover'), 'tutorial demo playback must switch to a compact mobile coach bar instead of blocking the operation area')
  assert(fs.readFileSync(path.join(root, 'src/constants/tutorialDemo.ts'), 'utf8').includes('TUTORIAL_REQUIREMENT_DEMO_DURATION_MS = 3200'), 'tutorial requirement demo should be fast enough for mobile preview without feeling stalled')
