...
async function checkProfileContracts() {
  ...
  assert(tutorialOverlay.includes('tutorial-demo-toast') && globalCss.includes('.tutorial-demo-playing .tutorial-spotlight') && globalCss.includes('.tutorial-demo-playing .tutorial-demo-toast'), 'tutorial demo playback must hide blocking guidance layers and show only a tiny status pill on mobile')
  assert(fs.readFileSync(path.join(root, 'src/constants/tutorialDemo.ts'), 'utf8').includes('TUTORIAL_REQUIREMENT_DEMO_DURATION_MS = 2600'), 'tutorial requirement demo should be fast enough for mobile preview without feeling stalled')
