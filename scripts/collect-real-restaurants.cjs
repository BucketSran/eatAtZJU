...
function candidateToRestaurant(candidate, index) {
  ...
  const restaurant = {
    ...
    studentScore: Math.max(65, Math.min(96, Math.round(62 + confidence * 28 + Math.max(0, 5 - (candidate.distanceMeters / 1000)) * 1.5))),
    checkins: Math.max(1, Math.round((candidate.rating || 4) * 30 + Math.max(0, radiusMeters - candidate.distanceMeters) / 40)),
