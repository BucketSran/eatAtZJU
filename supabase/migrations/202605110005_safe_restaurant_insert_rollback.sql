...
begin
  ...
  if v_before_data is null then
    if v_table = 'restaurants' and (
      exists (select 1 from public.dishes where restaurant_id = v_target_id)
      or exists (select 1 from public.reviews where restaurant_id = v_target_id)
    ) then
      raise exception 'Rollback would delete dependent restaurant content; archive or review dependencies first' using errcode = '23503';
    end if;
